...
if __name__ == '__main__':
    '''
    file1 = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/ak_fire/FireAreaHistory_gt500ha.shp"
    file2 = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/can_fire/" \
            "NFDB_poly_20171106_gt500ha_ABoVE_geo_boreal.shp"

    outfile1 = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/FireAreaHistory_gt500ha_multi.shp"
    outfile1_ = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/FireAreaHistory_gt500ha_multi.csv"

    outfile2 = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/NFDB_poly_20171106_gt500ha_multi.shp"
    outfile2_ = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/NFDB_poly_20171106_gt500ha_multi.csv"
    ...
    wkt_marker = list(0 for _ in vec1.wktlist)
    ...
    for i in range(nsamp):
        for j in range(i, nsamp):
            if (wkt_marker[i] + wkt_marker[j] <= 1) and (i != j):
                ...
                if (Vector.get_osgeo_geom(vec1.wktlist[i])).Intersects(Vector.get_osgeo_geom(vec1.wktlist[j])):
                    ...
                    intersect_count += 1
                    print('Ak {} : {} : {} : True'.format(str(i+1), str(j+1), str(intersect_count)))

    intersect_mark = (np.where(np.array(wkt_marker) > 0)[0]).tolist()

    vec1_ = vec1
    vec1_.wktlist = list(vec1.wktlist[i] for i in intersect_mark)
    vec1_.attributes = list(vec1.attributes[i] for i in intersect_mark)

    vec1_.write_vector(outfile1)


    file2 = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/can_fire/" \
            "NFDB_poly_20171106_gt500ha_ABoVE_geo_boreal_s3.shp"
    outfile2 = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/FIRE/NFDB_poly_20171106_gt500ha_multi.shp"
