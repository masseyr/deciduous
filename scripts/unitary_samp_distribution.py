...
def md_clean(sample_dicts,
    ...
    if len(sample_dicts) > len(bandnames):
        ...
        out_samples = list(sample_dicts[i] for i in loc)
...
def reformat_samples(site_dicts):
    ...
    for site_dict in site_dicts:
        out_dict = copy.deepcopy(site_dict)
        ...
        for samp_dict in data:
            if len(samp_dict) != 0:
                ...
                temp_dict = dict()
                temp_dict.update(samp_data)
                temp_dict.pop('bands')
                temp_dict.update(samp_data['bands'])
...
def ndvi_pctl_composite(site_dict,
    ...
    :param site_dict: Dictionary of Extracted Landsat values for a site
    ...
    if len(ndvi_list) > 0:
        ...
        out_dict = copy.deepcopy(site_dict[index])
...
if __name__ == '__main__':
    ...
    bandnames = ['ndvi_1', 'ndvi_2', 'ndvi_3']
    ...
    outfile = folder + "gee_data_cleaning_v{}.csv".format(str(version))
    ...
    print('Reduced samples by composite year medians: {}'.format(str(len(all_samp))))
    ...
    # clean all samples and append all the samples to one list
    all_cut_samp = list()
