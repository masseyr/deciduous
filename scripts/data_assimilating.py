...
cols = ['PSP Number', 'UTM Easting', 'UTM Northing', 'UTM Zone', 'Datum',
        't0', 't1', 't2', 't3', 't4', 't5',
        'sp1_ID_BA_t0', 'sp2_ID_BA_t0', 'sp3_ID_BA_t0', 'sp4_ID_BA_t0',
        ...
        'sp1_FracBA_t0', 'sp2_FracBA_t0', 'sp3_FracBA_t0', 'sp4_FracBA_t0',
...
for index, row in samp_data.iterrows():
    ...
    for time_stamp in time_headers:
        ...
        if not math.isnan(row[time_stamp]):
            ...
            for i, specie_id in enumerate(specie_header_list):
                if type(row[specie_id]).__name__ == 'str':
                    ...
                    frac = frac + float(row[specie_ba_header_list[i]])*float(specie_type[row[specie_id]])
                else:
...
retain_zero_percent = 10

num_retain = int(round(float(retain_zero_percent*count_non_zeroes)/float(100.0-retain_zero_percent)))
