...
samp_file = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/excel/CAFI_PSP.csv"
out_file = "D:/Shared/Dropbox/projects/NAU/landsat_deciduous/data/excel/CAFI_PSP.shp"
    ...
    for time_stamp in time_headers:
        ...
        if not math.isnan(row[time_stamp]):
            ...
            for i, specie_header in enumerate(specie_header_list):
                if type(row[specie_header]).__name__ == 'float' and not math.isnan(row[specie_header]):
                    ...
                    frac = frac + float(row[specie_ba_header_list[i]])*float(specie_id_cafi[row[specie_header]])
                else:
