...
if __name__ == '__main__':
    ...
    folder = 'c:/users/rm885/Dropbox/projects/NAU/landsat_deciduous/data/SAMPLES/fires/'
    filelist = [  # 'can_fire_multi_s3_extract_v2019_08_27T20_22_56.csv',
                'can_fire_multi_s2_extract_v2019_08_27T20_37_27.csv']
    ...
    for file_base_name in filelist:
        ...
        for elem in list_dicts:
            elem_list = [elem['FIREID'].replace('-', '').replace('_', '')\
                         .replace(' ', '').replace('(', '')\
                         .replace(')', '').replace('+', '') +
                         str(elem['YEAR']) +
